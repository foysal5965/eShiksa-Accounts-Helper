...
export const drawTable = (page, font, boldFont, x, y, bill, college, rgb) => {
  ...
  // Dynamic Data
  const admissionMsg = bill.admissionMsg ? bill.admissionMsg : 0
  const groupMsg = bill.groupMsg ? bill.groupMsg : 0
  const proReMigraMsg = bill.proReMigraMsg ? bill.proReMigraMsg : 0
  const professionalAddMsg = bill.professionalAddMsg ? bill.professionalAddMsg : 0
  const stdNtsMsg = bill.groupMsg ? bill.stdNtsMsg : 0
  const TutionFeeMsg = bill.TutionFeeMsg ? bill.TutionFeeMsg : 0
  const absentMsg = bill.absentMsg ? bill.absentMsg : 0
  const softwareFee = bill.cloudSpaceUnit * college.cloudSpacePricePerUnit;
  const smsCount =
    admissionMsg +
    groupMsg +
    proReMigraMsg +
        ...
        if (i === 3 || i === 4) { // Only draw 4th and 5th columns (Total Amount row)
          ...
          lines.forEach((line, idx) => {
            page.drawText(line, {
              ...
              size: fontSize,
              font: boldFont,  // Use bold font for "Total Amount" row
              color: rgb(0, 0, 0),
            });
          });
        }
